...
export async function POST(req: Request) {
  try {
    ...
    const sale = await upsertSale({
      ...
      externalId: orderId,
      status,
      grossAmount: order.value ? parseFloat(order.value) : (order.total ? parseFloat(order.total) : 0),
      netAmount: order.value ? parseFloat(order.value) : (order.total ? parseFloat(order.total) : 0),
      currency: 'BRL',
    ...
    else if (alias.includes('pix') || order.payment_method === 'pix') notifType = 'pix_pending'

    await createSaleNotification({
      workspaceId,
      type: notifType,
      amount: order.value ? parseFloat(order.value) : (order.total ? parseFloat(order.total) : 0),
      currency: 'BRL',
      platform: 'Yampi',
      saleId: sale.id,
